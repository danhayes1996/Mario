...
public class Brick extends Tile {
	...
	private static final ColourSet owCols = new ColourSet(0, 0xffffffff, 0xffBDBDBD, 0xff7B7B7B);
	
	public Brick(int x, int y) {
		this(x, y, 1, 1);
	}
	
	public Brick(int x, int y, int repeatWidth, int repeatHeight) {
		this.x = x;
		this.y = y;
	...
	public static List<GameObject> createFromString(String src) {
		...
		return Arrays.asList(new Brick(intArgs[0], intArgs[1], intArgs[2], intArgs[3]));
	}

	public void render(Screen screen) {
		screen.renderRepeating((int)x, (int)y, 32, 16, 8, 16, owCols, 2 * repeatWidth, repeatHeight);
